...
function Header({ isLoggedIn, loginMsg }) {
    return (
        <ul className="header">
            ...
            <li>
                <NavLink activeClassName="active" to="/quickQuote">Quick Quote</NavLink>
            </li>
            <li>
...
function QuickQuote() {
    ...
    useEffect(() => {
        setData(null);
        loginFacade.fetchQuickQuoteData().then(res => setData(res))
            .catch(err => {
                if (err.status) {
                    console.log(err.message);
                }
    ...
    const quote = data ? (
        <div>
            <p>Quote from Trump</p>
            <em>{data.value}</em>
            <p>Reference: https://tronalddump.io/random/quote</p>
        </div>
    ) : "Loading..."
    return (
        <div>
